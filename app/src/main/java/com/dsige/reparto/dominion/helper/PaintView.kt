...
class PaintView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
    ...
    fun save(context: Context, id: Int, tipo: Int, f: String): String {
        val nameImg: String = if (f.isEmpty()) {
            Util.getDateFirmReparto(id, tipo)
        } else {
            Util.getDateFirmReconexiones(id, tipo, f)
        }

        val image = File(Util.getFolder(context), nameImg)
        ...
        val x = 20f
        var y: Float = (mBitmap.height - bounds.height() * noOfLines).toFloat()
        ...
        // Tamaño del Fondo
        val top = mBitmap.height - bounds.height() * (noOfLines + 1)
